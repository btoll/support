...
create_home() {
    ...
    read -p "Install Directory [~/ie-virtualmachines]: " ievms_home

    def_ievms_home="${HOME}/ie-virtualmachines"
    ievms_home=${ievms_home:-$def_ievms_home}
    mkdir -p "${ievms_home}"
    cd "${ievms_home}"
}
...
echo -e "\nThis script will download and install IE virtual machines.\n"
echo -e "Specify version(s). Version numbers are space delimited.\n"
read -p "Press ENTER for defaults. [7 8 9]: " all_versions

default_versions="7 8 9"
all_versions=${all_versions:-$default_versions}
